...
const dbMigrate = async () => {
  try {
    ...
    for (const folder of migrationFolders) {
      ...
      const configFile = fse.readFileSync(configFilePath)
      const notCommentedConfigFile = configFile
        .toString()
        .replace("// migrations:", "migrations:")

      // write not commented config file
      fse.writeFileSync(configFilePath, notCommentedConfigFile)

      console.info(` | Migrating ${folder} database`)
      await exec(
        `cross-var npm run typeorm -- migration:run -d ${configFilePath}`,
        "./",
      )

      // write commented config file
      fse.writeFileSync(configFilePath, configFile)
